...
**서버(Vercel KV)에 저장**되어 모든 방문자에게 함께 보입니다.
...
### 5. 게시판용 Vercel KV(데이터베이스) 연결 — 필수

게시판이 작동하려면 서버에 데이터를 저장할 저장소가 하나 필요합니다.
Vercel이 제공하는 KV(Redis) 저장소를 프로젝트에 연결해주세요. (무료 한도 안에서
사용하면 별도 비용이 들지 않습니다.)

1. Vercel 대시보드 → 해당 프로젝트 → 상단 **Storage** 탭 클릭
2. **Create Database → KV** 선택 후 이름을 정하고 생성
3. 생성된 KV 데이터베이스 화면에서 **Connect Project** 클릭 → 이 프로젝트 선택 → 연결
4. 연결하면 `KV_REST_API_URL`, `KV_REST_API_TOKEN` 등 필요한 환경변수가
